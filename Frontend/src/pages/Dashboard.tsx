...
const Dashboard = () => {
  ...
  const recentResults =
    useLiveQuery(() =>
      db.results.orderBy("createdAt").reverse().limit(5).toArray(),
    ) || [];
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1 text-sm sm:text-base">
            Here is the overview for {config?.schoolName}.
          </p>
        </div>
        <Link
          to="/results/create"
          className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 flex items-center gap-2 w-full sm:w-auto justify-center"
        >
          <FilePlus size={18} />
          Create New Result
        </Link>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        <div className="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-sm border border-gray-100 dark:border-gray-800 flex items-center gap-4">
          <div className="p-4 bg-green-50 dark:bg-green-900/30 rounded-full shrink-0">
            <CheckCircle
